    ...
    Backend.get(projectId + "/launch/statistics?" + Utils.filterToQuery(filter))
      .then(response => {
        ...
        if (typeof response.all !== 'undefined') {
          const userSeries = [{ name: "Statuses", data: Object.keys(response.all.users).map(user => ({ name: user, y: response.all.users[user] })) }];
          Highcharts.chart({
            chart: { type: "pie", renderTo: "pie-by-users" },
            title: { verticalAlign: "middle", floating: true, text: "Users", style: { fontSize: "10px" } },
            plotOptions: { pie: { dataLabels: { format: "{point.name}: {point.percentage:.1f} %" }, innerSize: "70%" } },
            series: userSeries,
          });
        }
      })
