...
const emptyTestcase = () => ({
  id: null, importedName: "", description: "", steps: [], attributes: {},
  attachments: [], results: [], comments: [], properties: [], broken: false, locked: false, displayErrorMessage: "",
});

function TestCase({ testcase: testcaseProp, testcaseId, projectId: projectIdProp, match,
                    onProjectChange, onSessionChange, launchId: launchIdProp, testDeveloper: testDeveloperProp,
                    readonly: readonlyProp, callback, projectAttributes: projectAttrsProp }) {
  ...
  const historyFilter = useMemo(() => ({
    ...
    entityType: "TestCase", entityId: testcase.id,
  ...
  function getTestCase(pid, tcId) {
    ...
    Backend.get(pid + "/testcase/" + tcId)
      ...
      .catch(() => { setErrorMessage("Couldn't fetch testcase"); setLoading(false); });
  }
  ...
  function cancelEdit(fieldName, event, index) {
    ...
    if (index != undefined) {
      ...
      if (editorInstances.current[actionKey]) editorInstances.current[actionKey].setContent(originalTestcase[fieldName]?.[index]?.action || "");
      if (editorInstances.current[expKey]) editorInstances.current[expKey].setContent(originalTestcase[fieldName]?.[index]?.expectation || "");
    } else {
      if (editorInstances.current[fieldName]) editorInstances.current[fieldName].setContent(originalTestcase[fieldName] || "");
    }
  ...
  function editAttributeValues(key, values) {
    setOriginalTestcase(prev => ({ ...prev, attributes: { ...prev.attributes, [key]: (testcase.attributes || {})[key] } }));
  ...
  function cancelEditAttributeValues(event, key) {
    ...
    setAttributesInEdit(prev => { const s = new Set(prev); s.delete(key); return s; });
  ...
  function removeAttribute(key, event) {
    setTestcase(prev => { const attrs = { ...prev.attributes }; delete attrs[key]; return { ...prev, attributes: attrs }; });
    setAttributesInEdit(prev => { const s = new Set(prev); s.delete(key); return s; });
  ...
  function addAttribute(event) {
    ...
    setAttributesInEdit(prev => { const s = new Set(prev); s.add(null); return s; });
  }
  ...
  function editAttributeKey(key, data, reRender) {
    ...
    setAttributesInEdit(prev => { const s = new Set(prev); s.delete(key); s.add(data.value); return s; });
    setTestcase(prev => {
  ...
  function toggleEditAttribute(attributeId) {
    setAttributesInEdit(prev => { const s = new Set(prev); s.add(attributeId); return s; });
  }
  ...
  function removeTestcase() {
    Backend.delete(projectIdRef.current + "/testcase/" + testcase.id)
      .then(() => { window.location.href = window.location.href.replace("testcase=" + testcase.id, ""); })
  ...
  function cloneTestCase() {
    Backend.post(projectIdRef.current + "/testcase/" + testcase.id + "/clone")
      .then(response => { window.location.href = window.location.href.replace('testcase=' + testcase.id, 'testcase=' + response.id); })
  ...
  return (
    <div>
      ...
      <ul className="nav nav-tabs" id="tcTabs" role="tablist">
        <li className="nav-item" onClick={e => { e.preventDefault(); setActiveTab('main'); }}>
          <a className={`nav-link${activeTab === 'main' ? ' active' : ''}`} id="main-tab" href="#" role="tab" aria-controls="home" aria-selected={activeTab === 'main'}>Main</a>
        </li>
        {testcase.failureDetails && Object.keys(testcase.failureDetails).length > 0 && (
          <li className="nav-item" onClick={e => { e.preventDefault(); setActiveTab('failure'); }}>
            <a className={`nav-link${activeTab === 'failure' ? ' active' : ''}`} id="failure-tab" href="#" role="tab">Failure</a>
          </li>
        )}
        {!launchId && (
          <li className="nav-item" onClick={e => { e.preventDefault(); setActiveTab('attachments'); }}>
            <a className={`nav-link${activeTab === 'attachments' ? ' active' : ''}`} id="attachments-tab" href="#" role="tab">
              Attachments
        ...
        {launchId && (
          <li className="nav-item" onClick={e => { e.preventDefault(); setActiveTab('results'); }}>
        ...
        {launchId && (
          <li className="nav-item" onClick={e => { e.preventDefault(); setActiveTab('comments'); }}>
        ...
        {testcase.metaData && Object.keys(testcase.metaData).length > 0 && (
          <li className="nav-item" onClick={e => { e.preventDefault(); setActiveTab('metadata'); }}>
            <a className={`nav-link${activeTab === 'metadata' ? ' active' : ''}`} id="metadata-tab" href="#" role="tab">Metadata</a>
          </li>
        )}
        <li className="nav-item" onClick={e => { e.preventDefault(); setActiveTab('history'); }}>
          <a className={`nav-link${activeTab === 'history' ? ' active' : ''}`} id="history-tab" href="#" role="tab">History</a>
        </li>
      </ul>
        ...
        <div className={`tab-pane fade${activeTab === 'main' ? ' show active' : ''}`} id="main" role="tabpanel" aria-labelledby="main-tab">
          <div id="name" className="testcase-section">
            <div id="name-display" className="inplace-display row">
                ...
                <div className="col-1">
                  <div className="dropdown">
                    ...
                    <div className="dropdown-menu" aria-labelledby="dropdownMenuLink">
                      <a className="dropdown-item" href="#" onClick={() => cloneTestCase()}>Clone</a>
                    </div>
                  </div>
                </div>
              )}
              {!readonly && testcase.launchStatus !== undefined && (
                <div className="col-2">
                  <Checkbox toggle onChange={onBrokenToggle} checked={testcase.broken}
              ...
              <div id="name-form" className="inplace-form" style={{ display: "none" }}>
                <form>
                  <div className="form-group row">
                    ...
                    <div className="col-4">
                      <button type="button" className="btn btn-light" onClick={e => cancelEdit("name", e)}>Cancel</button>
                      <button type="button" className="btn btn-primary" onClick={e => handleSubmit("name", e)}>Save</button>
                    </div>
                  </div>
                </form>
              </div>
          ...
          <div id="description" className="card mb-4">
            ...
            <div className="card-body">
              <div id="description-display" className="inplace-display" dangerouslySetInnerHTML={{ __html: testcase.description }} />
              {!readonly && (
                <div id="description-form" className="inplace-form" style={{ display: "none" }}>
                  ...
                  <form>
                    <div className="testcase-inplace-buttons-down">
                      ...
                      <button type="button" className="btn btn-primary" onClick={e => handleSubmit("description", e)}>Save</button>
                    </div>
                  </form>
                </div>
              )}
          ...
          <div id="preconditions" className="card mb-4">
            ...
            <div className="card-body">
              <div id="preconditions-display" className="inplace-display" dangerouslySetInnerHTML={{ __html: testcase.preconditions }} />
              {!readonly && (
                <div id="preconditions-form" className="inplace-form" style={{ display: "none" }}>
                  ...
                  <form>
                    <div className="testcase-inplace-buttons-down">
                      ...
                      <button type="button" className="btn btn-primary" onClick={e => handleSubmit("preconditions", e)}>Save</button>
                    </div>
                  </form>
                </div>
              )}
          ...
          <div id="steps" className="mb-4">
            <LaunchTestcaseControls testcase={testcase} launchId={launchId} projectId={projectId} callback={callback} indicator={"START"} />
            <h5>Steps</h5>
              ...
              if (!step || step._new) {
                ...
                return (
                  <div key={i}>
                          ...
                          <div className="card-body">
                            {testcase.launchStatus && testcase.launchStatus.includes("RUNNING") ? (
                              <div className="card-text">
                                <div onClick={handleOnClickToSelectText}
                                  dangerouslySetInnerHTML={{ __html: "<b><i>" + (i + 1) + ". Step </i></b>" + step.action }} />
                              </div>
                            ) : (
                              <div className="card-text">
                                <div dangerouslySetInnerHTML={{ __html: "<b><i>" + (i + 1) + ". Step </i></b>" + step.action }} />
                              </div>
                            )}
                            ...
                            {!readonly && (
                              <div className="row">
                                ...
                                <div className="col-md-1">
                                  <a href="#" className="card-link" onClick={e => toggleEdit("steps", e, i)}>Edit</a>
                                </div>
                                <div className="col-md-1">
                                  <ConfirmButton onSubmit={removeStep} buttonClass={"card-link red float-right"}
                                    id={i} modalText={"Are you sure you want to remove Test Step?"} buttonText={"Remove"} />
                                </div>
                              </div>
                            )}
                          </div>
                    ...
                    {!readonly && (
                      <div id={"steps-" + i + "-form"} className="inplace-form card col-md-12" style={{ display: "none" }}>
                        <div className="card-body">
            ...
            {!readonly && (
              <div><button type="button" className="btn btn-primary" onClick={addStep}>Add Step</button></div>
            )}
          </div>

          <LaunchTestcaseControls testcase={testcase} launchId={launchId} projectId={projectId} callback={callback} indicator={"FAILUREDETAILS"} />
          ...
          <div id="attributes" className="mb-4">
                  ...
                  <div key={i} className="form-group attribute-block">
                      ...
                      <div className="card">
                        <div className="card-header">
                          <b>
                            ...
                            {!readonly && (
                              <span className="edit edit-icon clickable" onClick={() => toggleEditAttribute(attributeId)}>
                                <FontAwesomeIcon icon={faPencilAlt} />
                              </span>
                            )}
                            {!readonly && (
                    ...
                    {!readonly && (
                      <div id={"attributes-" + attributeId + "-form"} className="inplace-form"
                        ...
                        <form>
                          <div className="card">
                            <div className="card-header"><b>{getAttributeName(attributeId)}</b></div>
                            <div className="card-body">
                              <CreatableSelect
                              ...
                              <button type="button" className="btn btn-light" onClick={e => cancelEditAttributeValues(e, attributeId)}>Cancel</button>
                              <button type="button" className="btn btn-primary" onClick={e => handleSubmit("attributes", e, attributeId, true)}>Save</button>
                            </div>
                          </div>
                        </form>
                      </div>
                ...
                return (
                  <div key={i} className="form-group attribute-block">
                    <div id={"attributes-" + attributeId + "-form"} className="inplace-form">
                      <div className="card">
                        <div className="card-header">
                          <CreatableSelect onChange={e => editAttributeKey(attributeId, e, true)} options={getAttributeKeysToAdd()} />
                          <button type="button" className="btn btn-light" onClick={e => cancelEditAttributeKey(e, attributeId)}>Cancel</button>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            {!readonly && Utils.isAdmin(session) && (
              <div><button type="button" className="btn btn-primary" onClick={addAttribute}>Add Attribute</button></div>
            )}
          </div>
        </div>
        ...
        <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", marginRight: "10px" }}>
          {!readonly && Utils.isAdmin(session) && !testcase.locked && (
            <div style={{ marginRight: "10px" }}>
              <ConfirmButton onSubmit={lockTestcase} buttonClass={"btn btn-danger"} id={"testcase-lock"}
                modalText={"Are you sure you want to lock this test case?"} buttonText={"Lock Testcase"} />
            </div>
          )}
          {!readonly && Utils.isAdmin(session) && testcase.locked && (
            <div style={{ marginRight: "10px" }}>
              <ConfirmButton onSubmit={unlockTestcase} buttonClass={"btn btn-danger"} id={"testcase-unlock"}
                modalText={"Are you sure you want to unlock this test case?"} buttonText={"Unlock Testcase"} />
            </div>
          )}
          {!readonly && (
            <ConfirmButton onSubmit={removeTestcase} buttonClass={"btn btn-danger"} id={"testcase-removal"}
              modalText={"Are you sure you want to remove the Test Case?"} buttonText={"Remove Testcase"} />
          )}
        </div>
      </div>
    </div>
